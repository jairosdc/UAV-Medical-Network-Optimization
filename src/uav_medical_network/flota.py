...
Controlador de la flota de drones durante la simulación.

GestorFlotaController orquesta todas las operaciones sobre los drones:
  - inicializar_flota    : crea los drones de base y hospitalarios
  - procesar_nuevo_pedido: intenta asignar el mejor dron disponible
  - procesar_evento_llegada_hospital : completa el pedido, programa vuelta o recarga
  - procesar_evento_aterrizaje_base  : el dron vuelve a base, recarga si es necesario
  - procesar_evento_fin_recarga      : dron disponible de nuevo
  - obtener_resumen_estado           : contadores de drones por estado y rol

Reglas de negocio:
  - Inventario → drones role="base" (base → hospital → base)
  - Órganos    → drones role="hospital" (hospital → origen → destino, sin vuelta)
